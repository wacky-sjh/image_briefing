  ...
  return (
    <div className="bg-white/80 rounded-2xl shadow-xl p-8 w-full max-w-md flex flex-col items-center">
      ...
      <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
        ...
        {imagePreview && <PreviewImage imagePreview={imagePreview} />}
        <SubmitButton loading={loading} />
      </form>
      {result && <ResultDescription result={result} />}
    </div>
  );
}
...
const PreviewImage = ({ imagePreview }: { imagePreview: string }) => {
  return (
...
const ResultDescription = ({ result }: { result: string | null }) => {
  return (
